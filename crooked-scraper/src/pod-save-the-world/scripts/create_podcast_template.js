...
const savePodcastItem = (TABLE_NAME, imageUrl) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        const params = {
            ...
            Item: {
                'PodcastName': { S: 'Pod Save the World' },
        ...
        db.putItem(params, (err, result) => {
            if (err) {
                console.log('***** FAILED TO SAVE POD SAVE THE WORLD PODCAST ITEM IN DB *****');
                console.log(`***** FAILURE DUE TO: ${err} *****`);
                reject(err);
            } else {
                console.log('***** POD SAVE THE WORLD PODCAST ITEM SUCCESSFULLY SAVED *****')
...
const getPodcastImage = (BUCKET_NAME) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        const req = https.get(process.env.POD_SAVE_THE_WORLD_IMAGE_URL, (res) => {

            s3.upload({
                ACL: 'private',
                Bucket: BUCKET_NAME,
                Key: 'PodSaveTheWorldImage.jpg',
                Body: res,
                ContentType: 'image/jpeg'
            }, (err, result) => {
                if (err) {
                    console.log('***** FAILED TO UPLOAD IMAGE OF POD SAVE THE WORLD *****');
                    console.log(`***** ERROR DUE TO: ${err} *****`);
                    reject(err);
                } else {
                    console.log('***** SUCCESSFULLY UPLOADED IMAGE OF POD SAVE THE WORLD *****');
                    console.log(result);
